...
describe ClearancingService do
  ...
  describe "process from CSV file" do
    ...
    context "total success" do
      ...
      before do
        @clearancing_status = described_class.from_csv_file(uploaded_file).process
      end
    ...
    context "partial success" do
      ...
      it "detects all errors generated by invalid items" do
        ...
        [ invalid_id, no_id, float_id ].each do |bad_id|
          expect(@clearancing_status.errors).to include("Item id '#{bad_id}' is not valid")
        end
        [ non_existent_id ].each do |bad_id|
          expect(@clearancing_status.errors).to include("Item id #{bad_id} could not be found")
        end
        [ unsellable_item.id ].each do |bad_id|
          expect(@clearancing_status.errors).to include("Item id #{bad_id} could not be clearanced")
        end
      end
